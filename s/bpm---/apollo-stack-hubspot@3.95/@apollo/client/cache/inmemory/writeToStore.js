...
var StoreWriter = /** @class */ function() {
    ...
    StoreWriter.prototype.processSelectionSet = function(_a) {
        ...
        this.flattenFields(selectionSet, result,
            ...
            } else if (process.env.NODE_ENV !== "production" && !context.clientOnly && !context.deferred && !addTypenameToDocument.added(field) &&
                // If the field has a read function, it may be a synthetic field or
                // provide a default value, so its absence from the written data should
                // not be cause for alarm.
                !policies.getReadFunction(typename, field.name.value)) {
                process.env.NODE_ENV !== "production" && invariant.error(12, resultKeyNameFromField(field), result);
            }
        });
    ...
    StoreWriter.prototype.processFieldValue = function(value, field, context, mergeTree) {
        ...
        if (!field.selectionSet || value === null) {
            ...
            return process.env.NODE_ENV !== "production" ? cloneDeep(value) : value;
        }
        if (isArray(value)) {
...
function warnAboutDataLoss(existingRef, incomingObj, storeFieldName, store) {
    ...
    process.env.NODE_ENV !== "production" && invariant.warn(14, fieldName, parentType, childTypenames.length ? "either ensure all objects of type " + childTypenames.join(" and ") + " have an ID or a custom merge function, or " : "", typeDotName, __assign({}, existing), __assign({}, incoming));
}