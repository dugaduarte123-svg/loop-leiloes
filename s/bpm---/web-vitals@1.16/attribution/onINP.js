...
import {
    whenIdleOrHidden
} from '../lib/whenIdleOrHidden.js';
import {
    onINP as unattributedOnINP
} from '../onINP.js';
...
export const onINP = (onReport, opts = {}) => {
    ...
    const saveInteractionTarget = interaction => {
        if (!interactionTargetMap.get(interaction)) {
            var _interaction$entries$;
            // Use find to get first selector
            const node = (_interaction$entries$ = interaction.entries.find(e => e.target)) === null || _interaction$entries$ === void 0 ? void 0 : _interaction$entries$.target;
            if (node) {
                ...
                var _interaction$entries$2;
                // Fall back to targetSelector
                const selector = (_interaction$entries$2 = interaction.entries.find(e => e.targetSelector)) === null || _interaction$entries$2 === void 0 ? void 0 : _interaction$entries$2.targetSelector;
                if (selector) {
    ...
    const attributeLoAFDetails = attribution => {
        ...
        if (!((_attribution$longAnim = attribution.longAnimationFrameEntries) !== null && _attribution$longAnim !== void 0 && _attribution$longAnim.length) || !interactionTime || !nextPaintTime) {
            return;
        }
