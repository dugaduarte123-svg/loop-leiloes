...
import {
    LayoutShiftManager
} from '../lib/LayoutShiftManager.js';
import {
    getLoadState
...
const getLargestLayoutShiftSource = sources => {
    return sources.find(s => {
        var _s$node;
        return ((_s$node = s.node) === null || _s$node === void 0 ? void 0 : _s$node.nodeType) === 1;
    }) || sources[0];
};
/**
...
export const onCLS = (onReport, opts = {}) => {
    ...
    layoutShiftManager._onAfterProcessingUnexpectedShift = entry => {
        var _entry$sources;
        if (entry !== null && entry !== void 0 && (_entry$sources = entry.sources) !== null && _entry$sources !== void 0 && _entry$sources.length) {
            const largestSource = getLargestLayoutShiftSource(entry.sources);
            const node = largestSource === null || largestSource === void 0 ? void 0 : largestSource.node;
            if (node) {
    ...
    const attributeCLS = metric => {
        ...
        if (metric.entries.length) {
            var _largestEntry$sources;
            const largestEntry = getLargestLayoutShiftEntry(metric.entries);
            if (largestEntry !== null && largestEntry !== void 0 && (_largestEntry$sources = largestEntry.sources) !== null && _largestEntry$sources !== void 0 && _largestEntry$sources.length) {
