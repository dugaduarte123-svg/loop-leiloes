...
const localVehicleImage = (vehicle) => (vehicle?.id || vehicle?.sku) && vehicle?.image ? `/media/vehicles/${vehicle.id || vehicle.sku}/${encodeURIComponent(vehicle.image)}` : '/app-assets/placeholder.webp';
...
function gallery(vehicle) {
  ...
  const paths = images.map((image) => `/media/vehicles/${vehicle.id}/${encodeURIComponent(image.filename)}`);
...
async function renderLot(eventId, lotId) {
  ...
  const images = vehicle.images || [];
  const imagePaths = images.map((image) => `/media/vehicles/${vehicle.id}/${encodeURIComponent(image.filename)}`);
  ...
  if (rail && eventLots.length > 30) {
    rail.insertAdjacentHTML('beforeend', eventLots.slice(30).map((item) => `<a class="lot-rail-item ${String(item.id) === String(lotId) ? 'active' : ''}" href="${escapeHtml(item.url || '#')}"><span><img loading="lazy" src="/media/vehicles/${item.vehicle?.id}/${encodeURIComponent(item.vehicle?.image || '')}" alt="Lote ${item.number}"></span><b>Lote ${item.number}</b></a>`).join(''));
  }
...
function renderAuth(mode) {
  ...
  app.innerHTML = `<section class="auth-wrap"><div class="auth-card"><h1>${register ? 'Crie sua conta' : 'Seja bem-vindo!'}</h1><p>${register ? 'Preencha seus dados para participar dos eventos.' : 'Para participar dos eventos você precisa estar logado e com o cadastro completo.'}</p><form id="auth-form">${register ? '<label>Nome completo<input class="field" name="name" required></label>' : ''}<label><span class="sr-only">E-mail</span><input class="field" type="text" name="email" placeholder="E-mail ou CPF/CNPJ" required></label><label class="password-field"><span class="sr-only">Senha</span><input class="field" type="password" name="password" placeholder="Senha" minlength="6" required><button type="button" aria-label="Exibir senha">◉̸</button></label>${register ? '' : '<a class="auth-link" href="/esqueci-minha-senha">Esqueci minha senha</a>'}<button class="button" type="submit">${register ? 'Cadastrar' : 'Entrar'}</button><p id="auth-message"></p></form><p class="auth-switch">${register ? 'Já possui uma conta? <a href="/login">Entrar</a>' : 'Ainda não tem uma conta? <a href="/cadastro">Cadastre-se</a>'}</p></div></section>`;
